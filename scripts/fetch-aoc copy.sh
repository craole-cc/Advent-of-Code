...
SCRIPT_HEAD="Advent of Code - Asset Fetcher"
...
#> Detect the actual last available day for a given year
#? Some years (like 2025) may have fewer than 25 days
detect_year_max_day() {
	year="$1"
	session_key="$2"
	current_year=$(date -u +%Y)
	...
	#> Quick check: if it's the current year during Advent, use calculated max
	...
	#> For past years, probe the server to find actual max day
	#? Start from 25 and work backwards to find first existing day
	day=25
	while [ "${day}" -ge 1 ]; do
		#> Try to fetch just headers (faster than full input)
		url="${AOC_BASE_URL}/${year}/day/${day}/input"
		http_code=$(curl -s -I -w '%{http_code}' -o /dev/null -b "session=${session_key}" "${url}" 2>/dev/null)

		#? If we get 200, this day exists
		if [ "${http_code}" = "200" ]; then
			printf "%d" "${day}"
			return
		fi

		#? If 404, try previous day
		if [ "${http_code}" = "404" ]; then
			day=$((day - 1))
			continue
		fi

		#> Assume standard 25 upon auth errors, we can't detect
		if [ "${http_code}" = "401" ] || [ "${http_code}" = "403" ]; then
			log_info "Warning: Authentication issue while detecting days for ${year}, assuming 25 days" >&2
			printf "25"
			return
		fi

		#? For rate limiting or server errors, assume 25 to be safe
		if [ "${http_code}" = "429" ] || [ "${http_code}" -ge 500 ]; then
			log_info "Warning: Server issue while detecting days for ${year}, assuming 25 days" >&2
			printf "25"
			return
		fi

		#> Try previous day for unknown errors
		day=$((day - 1))
	done

	#> Fallback: assume at least day 1 exists
	printf "1"
}
...
show_menu() {
	#> Build each menu line as a fully formatted string
	title="${SCRIPT_HEAD}"
	line1="$(printf '1) Latest puzzle (%d Day-%d)' "${max_year}" "${max_day}")"
	line2="$(printf '2) Specific day of current year (%d)' "${max_year}")"
	line3="3) Specific year and day"
	line4="$(printf '4) All missing data (%d-%d)' "${FIRST_YEAR}" "${max_year}")"
	line5="5) Show help"
	line6="6) Exit"

	#> Measure visible character length of each line
	h1_len=$(len "${title}")
	l1_len=$(len "${line1}")
	l2_len=$(len "${line2}")
	l3_len=$(len "${line3}")
	...
	#? longest line among header and options
	content_width=${h1_len}
	[ "${l1_len}" -gt "${content_width}" ] && content_width=${l1_len}
	[ "${l2_len}" -gt "${content_width}" ] && content_width=${l2_len}
	[ "${l3_len}" -gt "${content_width}" ] && content_width=${l3_len}
	[ "${l4_len}" -gt "${content_width}" ] && content_width=${l4_len}
	[ "${l5_len}" -gt "${content_width}" ] && content_width=${l5_len}
	[ "${l6_len}" -gt "${content_width}" ] && content_width=${l6_len}

	#> Calculate box width
	#? Content plus one leading/trailing space and the two vertical borders
	box_width=$((content_width + 4))

	#> Calculate horizontal line of '═' that fits between the vertical borders
	horiz=''
	i=0
	horiz_char='='
	while [ "${i}" -lt $((box_width - 2)) ]; do
		horiz="${horiz}${horiz_char}"
		i=$((i + 1))
	done

	#> Center title in content_width
	left_pad=$(((content_width - h1_len) / 2))
	right_pad=$((content_width - h1_len - left_pad))
	title_line=$(printf '%*s%s%*s' "${left_pad}" '' "${title}" "${right_pad}" '')
...
fetch_all_missing_data() {
	...
	log_info "Fetching all missing data from ${FIRST_YEAR} to ${max_year}"
	log_info "Detecting actual puzzle counts per year (some years like 2025 had fewer days)..."
	...
	# Calculate total puzzles actually checked
	total_puzzles=$((fetched_count + skipped_count + error_count))
